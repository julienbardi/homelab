#!/bin/bash
# install-wireguard-server.sh v1.1 — Julien's reproducible WireGuard setup
...
# Assumes UGOS environment — no package updates, no Docker
...
echo "🔐 Generating client keys for 10.4.0.4..."
CLIENT_PRIV=$(wg genkey)
CLIENT_PUB=$(echo "$CLIENT_PRIV" | wg pubkey)

echo "📝 Writing WireGuard config to $WG_CONF..."
cat > "$WG_CONF" <<EOF
# WireGuard server config — autogenerated v1.1
...
echo "✅ WireGuard server is running on port $WG_PORT with subnet $WG_SUBNET"
echo "📡 Advertising LAN subnet $LAN_SUBNET"
...
echo "📱 To onboard, scan this QR code:"
