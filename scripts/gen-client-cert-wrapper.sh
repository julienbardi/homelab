...
# NOTE: We override PATH only inside this wrapper; never modify system PATH on UGOS.
...
"$RUN_AS_ROOT" "$SCRIPT_DIR/generate-client-cert.sh" "$CN" ${FORCE_FLAG:+"$FORCE_FLAG"}
...
TMP_PATH=$("$RUN_AS_ROOT" mktemp -p /run -m 600 "homelab.XXXXXX")

printf "%s\n\n%s\n" "$SUBJECT" "${SHA256_RAW:-}" | "$RUN_AS_ROOT" tee "$TMP_PATH" >/dev/null
