...
output=$(run_as_root --preserve-env echo "Hello from preserve")
...
FOO="hello world"
output=$(FOO="$FOO" run_as_root --preserve-env bash -c 'echo "$FOO"')
[[ "$output" == "hello world" ]] || fail "spaces in args preserve"
pass "spaces in args preserve"

log "=== Test 8: chained commands (preserve) ==="
output=$(run_as_root --preserve-env bash -c 'echo FirstPreserve && echo SecondPreserve')
...
# shellcheck disable=SC2016
output=$(run_as_root --preserve-env bash -c 'FOO=bar; echo $FOO')
...
# shellcheck disable=SC2016
output=$(run_as_root --preserve-env bash -c 'echo ${FOO:-__UNSET__}' 2>/dev/null || true)
...
if [ "$output" = "bar" ]; then
  ...
  if command -v run_as_root >/dev/null 2>&1; then
    log "Diagnostic: run_as_root --preserve-env env -> $(run_as_root --preserve-env env 2>/dev/null | grep -E '^FOO=' || echo '<none>')"
  fi
