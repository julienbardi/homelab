...
if [ -z "$DST_HOST" ]; then
    ...
    if [ -e "$DST_PATH" ]; then
        dst_owner=$(stat -c %u "$DST_PATH" 2>/dev/null) || dst_owner=$(stat -f %u "$DST_PATH" 2>/dev/null) || dst_owner=""
        dst_group=$(stat -c %g "$DST_PATH" 2>/dev/null) || dst_group=$(stat -f %g "$DST_PATH" 2>/dev/null) || dst_group=""
        dst_mode=$(stat -c %a "$DST_PATH" 2>/dev/null) || dst_mode=$(stat -f %Lp "$DST_PATH" 2>/dev/null) || dst_mode=""
    fi

    [ -n "$dst_owner" ] && [ "$dst_owner" != "$OWNER" ] && owner_drift=1
    [ -n "$dst_group" ] && [ "$dst_group" != "$GROUP" ] && group_drift=1

    # Normalize modes safely for Dash
