...
if systemctl is-active --quiet headscale; then
    headscale version 2>/dev/null | xargs log "Headscale running, version:"
else
...
if systemctl is-active --quiet coredns; then
    ...
    if dig @${NAS_IP} device.tailnet +short >/dev/null 2>&1; then
        log "CoreDNS resolving tailnet domain correctly"
    else
        log "WARN: CoreDNS not resolving tailnet domain"
    fi
else
...
if systemctl is-active --quiet unbound; then
    ...
    if dig @${NAS_IP} . NS +dnssec +short >/dev/null 2>&1; then
        log "Unbound resolving root NS with DNSSEC"
    else
        log "WARN: Unbound not resolving root NS"
    fi
else
