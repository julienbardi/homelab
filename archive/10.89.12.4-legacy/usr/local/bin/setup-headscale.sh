...
# Ensure systemd override for running as headscale
override_dir="/etc/systemd/system/headscale.service.d"
override_file="$override_dir/override.conf"

desired_override=$(cat <<'EOF'
# ⚠️ AUTOGENERATED by /usr/local/bin/setup-headscale.sh
# Do not edit manually — changes may be overwritten by homelab automation
[Service]
User=headscale
Group=headscale
...
if [ ! -d "$override_dir" ]; then
    echo "📂 Creating systemd override directory: $override_dir"
    sudo mkdir -p "$override_dir"
fi

# Compare existing file with desired content
if [ ! -f "$override_file" ] || ! diff -q <(echo "$desired_override") "$override_file" >/dev/null; then
    echo "✍️ Writing systemd override file: $override_file"
    echo "$desired_override" | sudo tee "$override_file" >/dev/null
else
    echo "✅ Systemd override already up-to-date at $override_file"
fi

echo "🔄 Reloading systemd daemon to apply override"
