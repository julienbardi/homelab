...
#
# ⚠️ AUTOGENERATED by /home/julie/homelab/scripts/setup-homelab.sh
...
# ⚠️ AUTOGENERATED by setup-unbound.sh
# DO NOT EDIT — changes will be overwritten by homelab automation
...
if [ ! -f "$conf_file" ] || ! diff -q <(echo "$desired_conf") "$conf_file" >/dev/null; then
    echo "✍️ Writing Unbound config to $conf_file"
    echo "$desired_conf" | sudo tee "$conf_file" >/dev/null
else
    echo "✅ Unbound config already up-to-date at $conf_file"
fi
...
if ! output=$(sudo unbound-checkconf "$conf_file" 2>&1); then
    echo "❌ Command failed: sudo unbound-checkconf $conf_file"
    echo "Error output:"
    echo "$output"
    echo "⚠️ Skipping restart/reload due to invalid config"
    exit 1
fi
...
sudo chown root:unbound /etc/unbound/unbound_control.pem /etc/unbound/unbound_control.key
...
# AppArmor read‑rules block for Unbound remote‑control TLS key/cert files.
REQUIRED_TEXT="  /etc/unbound/unbound_server.{key,pem} r,
  /etc/unbound/unbound_control.{key,pem} r,
  # non-chrooted paths
  /etc/unbound/** r,
...
if ! sudo unbound-control status >/dev/null 2>&1; then
    echo "❌ Unbound did not start correctly"
    exit 4
fi

# Final test queries to confirm resolution works
if ! command -v dig >/dev/null; then
    echo "❌ dig command not found, please install dnsutils/bind-utils"
    exit 5
fi

echo "🔍 Testing Unbound resolution with dig (IPv4 loopback)..."
if ! ipv4_output=$(dig @127.0.0.1 google.com +short +time=2 +tries=1 2>&1); then
    echo "❌ Test query failed: dig @127.0.0.1 google.com +short +time=2 +tries=1"
    echo "Error output:"
    echo "$ipv4_output"
    exit 6
fi
if [ -z "$ipv4_output" ]; then
    echo "⚠️ IPv4 test query returned no result (possible upstream issue)"
else
    echo "✅ IPv4 test query succeeded, google.com resolved to:"
    echo "$ipv4_output"
fi

echo "🔍 Testing Unbound resolution with dig (IPv6 loopback)..."
if ! ipv6_output=$(dig @::1 google.com +short +time=2 +tries=1 2>&1); then
    echo "❌ Test query failed: dig @::1 google.com +short +time=2 +tries=1"
    echo "Error output:"
    echo "$ipv6_output"
    exit 7
fi
...
if [ -z "$ipv6_output" ]; then
    echo "⚠️ IPv6 test query returned no result (possible upstream issue)"
else
    echo "✅ IPv6 test query succeeded, google.com resolved to:"
    echo "$ipv6_output"
fi

# DNSSEC validation test
echo "🔍 Testing Unbound DNSSEC validation..."
if ! dnssec_output=$(dig @127.0.0.1 cloudflare.com +dnssec +short +time=2 +tries=1 2>&1); then
    echo "❌ DNSSEC test query failed: dig @127.0.0.1 cloudflare.com +dnssec +short +time=2 +tries=1"
    echo "Error output:"
    echo "$dnssec_output"
    exit 8
fi
...
if [ -z "$dnssec_output" ]; then
    echo "⚠️ DNSSEC test query returned no result (possible upstream issue)"
else
    echo "✅ DNSSEC test query succeeded, cloudflare.com resolved with DNSSEC signatures:"
    echo "$dnssec_output"
fi

# Negative DNSSEC test (should fail validation)
echo "🔍 Testing Unbound with deliberately broken DNSSEC domain..."
if ! bad_output=$(dig @127.0.0.1 dnssec-failed.org +dnssec +short +time=2 +tries=1 2>&1); then
    echo "✅ As expected, dnssec-failed.org failed DNSSEC validation"
else
    echo "⚠️ Unexpected result: dnssec-failed.org returned data"
    echo "$bad_output"
    exit 9
fi

echo "🎉 Unbound recursive setup complete"
