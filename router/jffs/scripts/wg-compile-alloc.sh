...

{
    ...
    # Stable per-(base,iface) slot allocation
    if (!(key in idx)) {
        idx[key] = ++count[iface]
    }
    ...
    if (v4 != "" && !(key in is_server)) {
        ...
        rows[++n] = base "\t" iface "\t" idx[key] "\t" addr "\t" v4
    }
    ...
    if (v6 != "" && !(key in is_server)) {
        ...
        addr = a[1]
        sub(/::.*$/, "::" (idx[key] + 1), addr)
        rows[++n] = base "\t" iface "\t" idx[key] "\t" addr "\t" v6
    }
}

END {
    print "base\tiface\tslot\taddr\tcidr"
    ...
    for (k in is_server) {
        ...
        if (v4 != "") {
            split(v4, a, "/")
            addr = a[1]
            print base "\t" iface "\t" idx[k] "\t" addr "\t" addr "/32"
        } else if (v6 != "") {
            split(v6, a, "/")
            addr = a[1]
            print base "\t" iface "\t" idx[k] "\t" addr "\t" addr "/128"
        }
    }
}
' "$IN_PLAN" >"$OUT_ALLOC"
