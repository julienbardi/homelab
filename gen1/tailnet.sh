...
# tailnet.sh
# ------------------------------------------------------------
# Generation 1 helper: manage Headscale tailnet
# Host: 10.89.12.4 (NAS / VPN node)
# Responsibilities:
#   - Create namespaces for family/collaborators
#   - Register devices into tailnet
#   - Generate client configs and QR codes
#   - Log degraded mode if registration fails
# ============================================================
...
log() {
    echo "$(date '+%Y-%m-%d %H:%M:%S') [tailnet] $*" | tee -a /var/log/tailnet.log
    logger -t tailnet "$*"
}
...
if [ -z "${DEVICE_NAME}" ]; then
    log "ERROR: No device name provided, usage: tailnet.sh <device-name>"
    exit 1
fi
