...
lint-semantic:
	@echo "🔍 Checking semantic invariants (permissive)..."
	...
	if [ -n "$$bad" ]; then \
		echo "⚠️  Install targets without verify targets:"; \
		echo "$$bad"; \
	fi

lint-semantic-strict:
	@echo "🚨 Enforcing semantic invariants..."
	@bad=$$(grep -R "^[[:space:]]*install-pkg-" mk/*.mk \
		| sed 's/:.*//' | sort -u \
		| while read f; do \
			grep -q "verify-pkg-" "$$f" || echo "$$f"; \
	...
	if [ -n "$$bad" ]; then \
		echo "❌ install targets missing verification:"; \
		echo "$$bad"; \
		exit 1; \
	fi
...
lint-scripts-partial:
	@echo "🔍 Checking shell syntax for tracked .sh files..."
	@command -v bash >/dev/null 2>&1 || { \
		echo "❌ bash not found (required for syntax check)"; exit 2; }
	@if [ -z "$(SH_FILES)" ]; then \
...
lint-shellcheck:
	@echo "🔍 Scanning tracked shell files (permissive)"
	@if ! command -v $(SHELLCHECK) >/dev/null 2>&1; then \
	  echo "🔍 shellcheck not installed; install with 'make deps' or set SHELLCHECK=..."; \
	else \
	  if [ -z "$(SH_FILES)" ]; then \
...
lint-shellcheck-strict:
	@echo "🔍 Scanning tracked shell files (strict)"
	...
	@if [ -z "$(SH_FILES)" ]; then \
		...
		if [ $$rc -ne 0 ]; then echo "❌ Errors/warnings detected"; exit $$rc; fi; \
	fi

# Spell checks: codespell (permissive) and aspell (permissive)
lint-spell:
	@echo "🔍 Running codespell and aspell (permissive)..."
	...
	@if command -v $(ASPELL) >/dev/null 2>&1; then \
	  ...
	  echo "🔍 aspell not installed; skipping aspell"; \
	fi

# Spell checks strict: fail if codespell or aspell find issues
lint-spell-strict:
	@echo "🚨 Running codespell and aspell (strict)..."
...
lint-headscale: ensure-run-as-root
	...
	@if [ -z "$(run_as_root)" ]; then \
	  ...
	  $(run_as_root) headscale configtest --config /etc/headscale/config.yaml || echo "🔍 Headscale configtest failed (permissive)"; \
	else \
	  echo "🔍 headscale binary not found; skipping headscale configtest"; \
	fi
...
lint-headscale-strict: ensure-run-as-root
	...
	@if [ -z "$(run_as_root)" ]; then \
	  ...
	  $(run_as_root) headscale configtest --config /etc/headscale/config.yaml || { echo "❌ Headscale config invalid"; exit 1; }; \
	fi

.PHONY: check-control-plane-reasoning
check-control-plane-reasoning:
	@echo "🔍 Checking for forbidden intent-level semantic reasoning in router shell scripts..."
	@set -eu; \
	if rg -n --hidden --no-ignore-vcs \
		...
		-e 'awk.*exit\(' \
		scripts/; then \
		echo "❌ intent-level semantic reasoning detected in router shell scripts"; \
		exit 1; \
	else \
		echo "✅ No forbidden semantic reasoning detected"; \
	fi

.PHONY: check-exec-surface
check-exec-surface:
	@echo "🔍 Checking executable surface..."
	@violations=$$( \
