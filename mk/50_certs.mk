# ============================================================
# mk/certs.mk — Certificate orchestration
# ============================================================
...
# Base actions
issue:
	@$(DEPLOY) issue || { echo "[make] ❌ issue failed"; exit 1; }

renew:
	@$(DEPLOY) renew FORCE=$(FORCE) || { echo "[make] ❌ renew failed"; exit 1; }

prepare: renew fix-acme-perms
	@$(DEPLOY) prepare || { echo "[make] ❌ prepare failed"; exit 1; }

# Deploy targets (pattern rule)
deploy-%: prepare
	@$(DEPLOY) deploy $* || { echo "[make] ❌ deploy-$* failed"; exit 1; }

# Validate targets (pattern rule)
validate-%:
	@$(DEPLOY) validate $* || { echo "[make] ❌ validate-$* failed"; exit 1; }

# All-in-one targets (pattern rule: renew + prepare + deploy + validate)
all-%: renew prepare deploy-% validate-%
	@$(DEPLOY) all $* || { echo "[make] ❌ all-$* failed"; exit 1; }

# Cert watch setup targets
setup-cert-watch-%:
	@export SCRIPT_NAME="setup-cert-watch-$*"; . scripts/lib/run_as_root.sh && \
...
deploy-cert-watch-%:
	@export SCRIPT_NAME="deploy-cert-watch-$*"; . scripts/lib/run_as_root.sh && \
	run_as_root install -m 0644 scripts/systemd/cert-reload@.service /etc/systemd/system/cert-reload@.service && \
	run_as_root install -m 0644 scripts/systemd/$*-cert.path /etc/systemd/system/$*-cert.path && \
	run_as_root systemctl daemon-reload && \
	run_as_root systemctl enable --now $*-cert.path
...
deploy-cert-watch-all:
	@export SCRIPT_NAME="deploy-cert-watch-all"; . scripts/lib/run_as_root.sh && \
	run_as_root install -m 0644 scripts/systemd/cert-reload@.service /etc/systemd/system/cert-reload@.service && \
