...
# --------------------------------------------------------------------
# WireGuard control-plane model:
# - Router: special bring-up via wgctl.sh router up (SSH, ip link, wg setconf)
# - NAS: generic bring-up via wgctl.sh nas up (wg-quick)
# Makefile orchestrates only; all logic lives in wgctl.sh.
...
.PHONY: \
    ...
    wg-restart \
    wg-status \
    router-ensure-wg-module router-firewall \
    wg-clean-out wg-clean-state \
...
# wg-up orchestrates router and NAS bring-up in parallel.
# No ordering constraints: router and NAS are independent.
wg-up: wg-up-router wg-up-nas
