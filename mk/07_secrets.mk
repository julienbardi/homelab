...
sops-init:
	@tmp="$$(mktemp)"; \
	printf "%s\n" \
		"# .sops.yaml (generated by sops-init)" \
		"creation_rules:" \
		"  - path_regex: \".*/secrets\\.enc\\.yaml$$\"" \
		"    age: $(SOPS_AGE_PUBKEY_EXPECTED)" \
		"format: yaml" \
		> "$$tmp"; \
	if ! cmp --silent "$$tmp" .sops.yaml 2>/dev/null; then \
		echo "⚙️ Updating .sops.yaml"; \
		mv "$$tmp" .sops.yaml; \
	else \
		if [ "$(VERBOSE)" -ge 1 ]; then echo "🟢 .sops.yaml already up-to-date"; fi; \
		rm "$$tmp"; \
	fi
