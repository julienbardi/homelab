...
# Base NAT script body — WG rules dynamically appended from wg-interfaces.tsv
...
ip6tables -D INPUT -j HOMELAB_INPUT 2>/dev/null || true
ip6tables -I INPUT 1 -j HOMELAB_INPUT
...
ip6tables -A HOMELAB_INPUT -p udp --dport 53 -j ACCEPT
ip6tables -A HOMELAB_INPUT -p tcp --dport 53 -j ACCEPT
	...
	@awk -F'\t' -v lan_nas="$(LAN_NAS)" '\
		...
		{ \
			...
			if (host == "nas") { \
				...
				printf "ip6tables -A HOMELAB_INPUT -p udp --dport %s -j ACCEPT\n", port; \
			} \
		} \
	...
	@{ \
		...
		env CHANGED_EXIT_CODE=$(INSTALL_IF_CHANGED_EXIT_CHANGED) \
			$(INSTALL_FILE_IF_CHANGED) \
				"" "" "$(TMP_ROUTER_NAT)" \
				$$ROUTER_ADDR $$ROUTER_SSH_PORT "$(ROUTER_SCRIPTS)/nat-start" \
...
printf "\n=== [ HOMELAB_INPUT (Router-local UDP Accept - IPv4) ] ===\n"
if iptables -S HOMELAB_INPUT >/dev/null 2>&1; then
	if ! iptables -L HOMELAB_INPUT -n --line-numbers | sed 1,2d | grep .; then
		echo "  (IPv4 Chain is empty)"
	fi
else
	echo "❌ IPv4 Chain HOMELAB_INPUT does not exist"
fi

printf "\n=== [ HOMELAB_INPUT (Router-local UDP Accept - IPv6) ] ===\n"
if ip6tables -S HOMELAB_INPUT >/dev/null 2>&1; then
	if ! ip6tables -L HOMELAB_INPUT -n --line-numbers | sed 1,2d | grep .; then
		echo "  (IPv6 Chain is empty)"
	fi
else
	echo "❌ IPv6 Chain HOMELAB_INPUT does not exist"
fi
