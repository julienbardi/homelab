...
router-require-arm64: | router-ssh-check
	@$(call WITH_SECRETS, \
		ssh "$(SSH_HOST_ROUTER)" uname -m | grep -q aarch64 \
	)
...
router-caddy-bin: | router-ssh-check router-require-arm64
	...
	@$(call WITH_SECRETS, \
		ssh "$(SSH_HOST_ROUTER)" "$(call one_line,$(ROUTER_CADDY_BIN_CMD))" \
	)
...
router-caddy-config: router-certs-install-caddy | router-require-arm64 router-ssh-check
	...
	# Ensure destination directory exists (required for IFCv3)
	@$(call WITH_SECRETS, \
		ssh "$(SSH_HOST_ROUTER)" '\
			set -e; \
			mkdir -p "$(dir $(ROUTER_CADDYFILE_DST))"; \
			chmod 755 "$(dir $(ROUTER_CADDYFILE_DST))"; \
		' \
	)
...
router-caddy-version: | router-ssh-check
	@$(call WITH_SECRETS, \
		ssh "$(SSH_HOST_ROUTER)" '"$(ROUTER_CADDY_BIN)" version' \
	)
...
router-caddy-restart: | router-ssh-check
	...
	@$(call WITH_SECRETS, \
		ssh "$(SSH_HOST_ROUTER)" '/jffs/scripts/caddy-reload.sh' \
	)
