...
router-require-arm64: | router-ssh-check
	@$(call WITH_SECRETS, \
		ssh $(SSH_OPTS) -p "$$ROUTER_SSH_PORT" "$$SSH_USER_ROUTER@$$ROUTER_ADDR" uname -m | grep -q aarch64 \
	)
...
router-caddy-bin: | router-ssh-check router-require-arm64
	...
	@$(call WITH_SECRETS, \
		ssh "$$SSH_USER_ROUTER@$$ROUTER_ADDR" -p "$$ROUTER_SSH_PORT" "$(call one_line,$(ROUTER_CADDY_BIN_CMD))" \
	)
...
router-caddy-config: router-certs-install-caddy | router-require-arm64 router-ssh-check
	...
	$(INSTALL_FILE_IF_CHANGED) \
		...
		"$$SSH_USER_ROUTER@$$ROUTER_ADDR" "$$ROUTER_SSH_PORT" \
...
router-caddy-version: | router-ssh-check
	@$(call WITH_SECRETS, \
		ssh -p "$$ROUTER_SSH_PORT" "$$SSH_USER_ROUTER@$$ROUTER_ADDR" '"$(ROUTER_CADDY_BIN)" version' \
	)
...
router-caddy-restart: | router-ssh-check
	...
	@$(call WITH_SECRETS, \
		ssh -p "$$ROUTER_SSH_PORT" "$$SSH_USER_ROUTER@$$ROUTER_ADDR" '/jffs/scripts/caddy-reload.sh' \
	)
